...
| OMP (`omp`, `ompk`) | lifecycle hooks when installed; otherwise screen manifest | state |
...
Bundled manifests live inside Herdr. Herdr also checks herdr.pkking.computer for remote manifest updates and applies valid per-agent rule updates automatically without requiring a Herdr restart. Remote manifests are stored in Herdr's state directory. Set `[update] manifest_check = false` to disable background remote manifest checks.
