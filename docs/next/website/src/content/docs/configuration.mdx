...
The default keymap is mostly prefix-first. `ctrl+pagedown` and `ctrl+pageup` are direct shortcuts that cycle tabs across spaces and wrap at both ends. Common defaults include:
